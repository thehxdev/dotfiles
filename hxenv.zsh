...
if ! command -v killall &>/dev/null; then
    function killall() {
        for p in $(pgrep $1); do kill -9 $p; done
    }
fi
...
export JAVA_HOME="$__HX_OPT_GLOBAL_PATH/jdk-25.0.3+9"
...
# FZF
__FZF_WALKER_SKIP=".git,.cache,.var,go,_global,vendor,node_modules,target,build,vcpkg"
